...
func initialize(c echo.Context) error {
	...
	for _, p := range paths {
		...
		c.Echo().Logger.Infof("エラー確認用ログ : %s", cmdStr)
...
func searchEstateNazotte(c echo.Context) error {
	...
	query := fmt.Sprintf(`SELECT * FROM estate FORCE INDEX (idx_latitude_longitude_popularity_desc) WHERE latitude <= ? AND latitude >= ? AND longitude <= ? AND longitude >= ? AND ST_Contains(ST_PolygonFromText(%s), ST_GeomFromText(CONCAT ('POINT(',latitude,' ',longitude,')'))) ORDER BY popularity_desc, id ASC`, coordinates.coordinatesToText())
...
// TAKI:「POLYGON((11111 22222,333333 444444))」みたいな文字列に変換
